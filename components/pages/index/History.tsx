...
const getCateInfo = (item: any, cate_dict: any, project_dict: any) => {
  let result = {
    title: '',
    taker: '',
    logo: ''
  ...
  if(item.cate_id){
    if(typeof(cate_dict[item.cate_id]) != "undefined"){
...
const fetchHistoryData = async (address: string, chain: string, timestamp: number=0, token: string='') => {
  ...
  const res = await axios.get("/api/debank/user/all_history_list?" + param).catch(function (error) {
    return { data: { data: {}}}
  })
  ...
  if(res.data.data && res.data.data.history_list){
    ...
    for(let item of history_list){
      let catInfo: any = getCateInfo(item, cate_dict, project_dict)
...
const History: FC = () => {
  ...
  const loadMore = () => {
    ...
    //setHistoryData(historyData.concat(moredata))
    //setLoadingMore(false)
  }
